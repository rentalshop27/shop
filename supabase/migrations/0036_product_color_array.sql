create or replace function public.normalize_product_colors(p_value jsonb)
...
begin
  ...
  if jsonb_typeof(p_value) = 'array' then
    ...
    loop
      if v_entry <> '' and not (v_entry = any(v_colors)) then
        v_colors := array_append(v_colors, v_entry);
      end if;
    end loop;
...
alter table public.products
  ...
  alter column primary_color type text[]
    using public.normalize_product_colors(to_jsonb(primary_color)),
  alter column primary_color set default '{}'::text[];
...
begin
  ...
  insert into public.products (
    ...
    public.normalize_product_colors(p_product->'primary_color'),
